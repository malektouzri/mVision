...
import com.mVision.myProject.utils.CharDetectOCR;
import com.mVision.myProject.utils.CommonUtils;
import com.mVision.myProject.view.TouchImageView;

import static com.mVision.myProject.utils.CommonUtils.info;
...
public class RecognizeTextActivity extends Activity {
	...
	Button btnStartCamera,btnToList;
	Button btnExit;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		btnToList = (Button) findViewById(R.id.toListButton);
		btnToList.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View view) {
				Intent in = new Intent(RecognizeTextActivity.this , PassportActivity.class);
				if(!info.equals(""))
	...
	public void displayResult(Bitmap imageBitmap, int top, int bot, int right, int left) {
		...
		if (processImg.parseBitmap(imageBitmap, top, bot, right, left)) {
			...
			info = recognizeResult.getText().toString();
			...
			image.setImageBitmap(imageBitmap);
			hideProcessBar();
			dialogBox("Can not recognize text from this image. Please try again", "Retry", "Exit", true);
		}
	}
